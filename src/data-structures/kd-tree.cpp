...
inline bool less( const point& a, const point &b, bool byX ) {
...
void partition( point a[], int lo, int hi, const int& k, bool byX ) {
  ...
  if ( less( a[mid], a[lo], byX ) ) swap( a[mid], a[lo] );
  if ( less( a[hi], a[lo], byX ) ) swap( a[hi], a[lo] );
  if ( less( a[hi], a[mid], byX ) ) swap( a[hi], a[mid] );
  ...
  for ( ;; ) {
    while ( less( a[ ++l ], pivot, byX ) ) ;
    while ( less( pivot, a[ --r ], byX ) ) ;
    if ( l < r ) swap( a[l], a[r] );
    else break;
  }
