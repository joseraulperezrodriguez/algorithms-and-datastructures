...
struct Binomial_Heap {
  ...
  node* binomial_heap_merge( node *h1, node *h2 ) {
    ...
    if ( x->degree < y->degree )
         start = x, x = x->sibling;
    else start = y, y = y->sibling;
    curr = start;
    
    while ( x && y ) {
      if ( x->degree < y->degree )
           curr->sibling = x, curr = x, x = x->sibling;
      else curr->sibling = y, curr = y, y = y->sibling;
    }
    
    while ( x ) curr->sibling = x, curr = x, x = x->sibling;
    while ( y ) curr->sibling = y, curr = y, y = y->sibling;
  ...
  node* binomial_heap_erase_min() {
    ...
    if ( !prev_y ) head = head->sibling;
    else prev_y->sibling = y->sibling;
  ...
  inline void swap( int& x, int& y ) { int t = x; x = y; y = x; }
  ...
  node* binomial_heap_erase( node *x ) {
    binomial_heap_decrease_key( x, -oo ); /* assuming there is no key < -2^30 */
